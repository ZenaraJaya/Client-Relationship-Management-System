...
class CalendarReminderService
{
    ...
    protected function buildNotificationHtml(string $subject, string $summary, array $details): string
    {
        try {
            ...
            Log::warning('Blade reminder template render failed. Using inline HTML fallback.', [
                'error' => $e->getMessage(),
            ]);

            return $this->buildInlineNotificationHtml($subject, $summary, $details);
        }
    }

    protected function buildInlineNotificationHtml(string $subject, string $summary, array $details): string
    {
        ...
        foreach ($details as $line) {
            ...
            $label = htmlspecialchars((string) ($parts[0] ?? ''), ENT_QUOTES, 'UTF-8');
            $value = htmlspecialchars((string) ($parts[1] ?? ''), ENT_QUOTES, 'UTF-8');

            $rows .= sprintf(
                '<tr>
                    <td style="padding:11px 13px;background:#f8fbff;border-bottom:1px solid #e7eef8;width:36%%;font-size:11px;color:#64748b;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;">%s</td>
                    <td style="padding:11px 13px;border-bottom:1px solid #e7eef8;font-size:14px;color:#0f172a;line-height:1.45;">%s</td>
                </tr>',
                $label,
                $value
            );
            ...
            <table role="presentation" width="640" cellpadding="0" cellspacing="0" style="max-width:640px;width:100%;background:#ffffff;border:1px solid #d8e2f2;border-radius:16px;overflow:hidden;">
                <tr>
                    <td style="padding:24px 26px;background:#0f172a;color:#ffffff;">
                        ...
                        <h1 style="margin:10px 0 0;font-size:24px;line-height:1.35;font-weight:700;color:#ffffff;">{$safeSubject}</h1>
                    </td>
                </tr>
                <tr>
                    <td style="padding:24px 26px;">
                        ...
                        <p style="margin:16px 0 18px;font-size:15px;line-height:1.65;color:#334155;">{$safeSummary}</p>
